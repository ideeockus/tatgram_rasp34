...
async def get_class_name(message: types.Message, state: FSMContext):
    ...
    # else:  # uncomment if want to add inline class chooser
    #     classes_choose_list = []
    ...
    #             class_name = InlineKeyboardButton(class_name, callback_data=class_name)
    #             classes_choose_list_kb.insert(class_name)
    #     if len(classes_choose_list) < 1:
    #         await message.reply("Я не нашел такого класса")
    #     elif len(classes_choose_list) >= 1:
    #         await message.answer("Выберите класс из списка", reply_markup=classes_choose_list_kb)
    #         return
    # await PupilsRaspReqStates.waiting_for_action.set()


# @dp.callback_query_handler(state=PupilsRaspReqStates.waiting_for_class_name)
# async def teacher_full_name_inline(callback_query: types.CallbackQuery, state: FSMContext):
#     class_name = callback_query.data
#     print("выбор", class_name, "с инлайн клавиатуры")
